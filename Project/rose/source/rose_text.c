...
ROSE_Text* ROSE_TextCreate(const char* string) {
	...
	for (usize i = 0; i < num_characters; i++) {
		...
		vertices[index + 0] = (ROSE_Vertex){ { i + 0, 0 }, { 1.0f, 1.0f, 1.0f, 1.0f }, { x,     1 } };
		vertices[index + 1] = (ROSE_Vertex){ { i + 1, 0 }, { 1.0f, 1.0f, 1.0f, 1.0f }, { x + w, 1 } };
		vertices[index + 2] = (ROSE_Vertex){ { i + 1,-1 }, { 1.0f, 1.0f, 1.0f, 1.0f }, { x + w, 0 } };
		vertices[index + 3] = (ROSE_Vertex){ { i + 0, 0 }, { 1.0f, 1.0f, 1.0f, 1.0f }, { x,     1 } };
		vertices[index + 4] = (ROSE_Vertex){ { i + 1,-1 }, { 1.0f, 1.0f, 1.0f, 1.0f }, { x + w, 0 } };
		vertices[index + 5] = (ROSE_Vertex){ { i + 0,-1 }, { 1.0f, 1.0f, 1.0f, 1.0f }, { x,     0 } };
	}
